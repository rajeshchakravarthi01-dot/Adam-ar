      ...
      <div className="glass-panel p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="px-3 py-1.5 glass-input rounded-lg text-xs font-semibold"
          >
      ...
      <div className="glass-panel rounded-2xl overflow-hidden shadow-xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs font-mono">
            <thead className="glass-inner text-neutral-300 font-semibold border-b border-white/10 font-sans text-[11px] uppercase tracking-wider">
              <tr>
                <th className="py-2.5 px-3 w-44 text-teal-300">Timestamp</th>
                <th className="py-2.5 px-3 w-24">Level</th>
                <th className="py-2.5 px-3 w-48">Event</th>
                <th className="py-2.5 px-3">Message</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5 text-neutral-200 text-[11px]">
              {filtered.length === 0 ? (
                <tr>
                ...
                filtered.map((l) => (
                  <tr key={l.id} className="hover:bg-white/5 transition-colors">
                    <td className="py-2 px-3 text-neutral-400">{l.created_at}</td>
                      ...
                      <span
                        className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-bold uppercase border ${
                          l.level === 'error'
                            ...
                            : 'glass-inner text-neutral-300 border-white/10'
                        }`}
                      >
                        {l.level}
                      </span>
                    </td>
                    <td className="py-2 px-3 font-bold text-neutral-100">{l.event}</td>
                    <td className="py-2 px-3 text-neutral-300 font-sans">{l.message}</td>
                  </tr>
                ))
              )}
            </tbody>
