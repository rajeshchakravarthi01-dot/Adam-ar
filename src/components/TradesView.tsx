...
export const TradesView: React.FC<TradesViewProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-5 rounded-2xl relative overflow-hidden shadow-xl">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          <form onSubmit={handleUploadSubmit} className="flex flex-col sm:flex-row items-center gap-3 flex-1 w-full">
            <div className="relative flex-1 w-full">
              <input
                ...
                className="block w-full text-xs text-neutral-300 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-gradient-to-r file:from-teal-400 file:to-emerald-500 file:text-slate-950 hover:file:brightness-110 cursor-pointer border border-white/10 rounded-xl p-1 glass-inner-subtle"
              />
            </div>
            ...
            <button
              ...
              className="w-full sm:w-auto px-6 py-2.5 bg-gradient-to-r from-teal-400 to-emerald-500 hover:brightness-110 disabled:opacity-50 text-slate-950 font-bold rounded-xl text-xs shadow-lg shadow-teal-500/20 transition-all active:scale-95 cursor-pointer flex items-center justify-center gap-2 shrink-0"
            >
          ...
          <button
            ...
            className="w-full sm:w-auto px-4 py-2.5 glass-inner-subtle hover:bg-white/10 text-neutral-200 font-bold rounded-xl text-xs transition-colors cursor-pointer flex items-center justify-center gap-2 border border-white/10 shrink-0"
        ...
        {uploadStatus && (
          <div className="mt-3 text-xs font-medium text-teal-200 bg-teal-400/10 p-2.5 rounded-lg border border-teal-400/25">
            {uploadStatus}
          </div>
        )}
      </div>
      ...
      <div className="glass-panel text-white rounded-2xl p-5 shadow-xl">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-white/10">
          <div>
            <div className="flex items-center gap-2">
              <span className="p-1.5 rounded-lg bg-teal-500 text-slate-950 font-bold shadow-md shadow-teal-500/20">
                <Layers className="w-4 h-4" />
              </span>
              <h2 className="text-base font-bold text-white tracking-wide">
                Trade Pre-Orders Console &amp; Call Audit Target Engine
              </h2>
              <span className="text-[10px] font-mono bg-teal-400/20 text-teal-300 border border-teal-400/30 px-2 py-0.5 rounded-full font-bold">
                4-MIN GAP ALGORITHM
              </span>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            <button
              ...
              className="px-3.5 py-2 bg-gradient-to-r from-teal-400 to-emerald-500 hover:brightness-110 disabled:opacity-50 text-slate-950 font-bold rounded-xl text-xs transition-all flex items-center gap-1.5 shadow-md shadow-teal-500/20 cursor-pointer"
            ...
            <button
              ...
              className="px-3 py-2 glass-inner-subtle hover:bg-white/10 text-teal-300 font-bold rounded-xl text-xs transition-colors flex items-center gap-1.5 border border-white/10 cursor-pointer"
            >
            ...
            {onNavigateToManualAudit && (
              <button
                ...
                className="px-3 py-2 bg-teal-400/15 hover:bg-teal-400/25 text-teal-300 font-bold rounded-xl text-xs transition-colors flex items-center gap-1.5 border border-teal-400/30 cursor-pointer"
                title="Audit missing trades via mail confirmations"
              >
                <MailCheck className="w-3.5 h-3.5 text-teal-400" />
        ...
        {matchingNotification && (
          <div className="mt-3 p-3 bg-teal-500/15 border border-teal-500/30 rounded-xl flex items-center justify-between gap-3 text-xs text-teal-300">
            <div className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
          <div className="glass-inner p-3.5 rounded-xl border border-white/10">
            ...
            <div className="text-[11px] text-neutral-400 mt-0.5">UCCs executed across dates</div>
          </div>
          ...
          <div className="glass-inner p-3.5 rounded-xl border border-white/10">
            ...
            <div className="text-[11px] text-neutral-400 mt-0.5">Individual fill executions</div>
          </div>

          <div className="glass-inner p-3.5 rounded-xl border border-teal-400/30 bg-teal-400/10">
            <div className="flex items-center gap-1.5 text-teal-300 text-xs mb-1 font-semibold">
            ...
            <div className="text-[11px] text-teal-300/80 mt-0.5">Exact orders from trade console</div>
          </div>
        ...
        {showClustersBreakdown && (
          <div className="mt-4 pt-4 border-t border-white/10 space-y-4">
            ...
            {preOrdersSummary?.daily_breakdown && preOrdersSummary.daily_breakdown.length > 0 && (
              <div>
                <h4 className="text-xs font-bold text-teal-300 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                ...
                <div className="overflow-x-auto rounded-xl border border-white/10 glass-inner">
                  <table className="w-full text-left text-xs">
                    <thead className="glass-inner text-neutral-400 text-[11px] uppercase border-b border-white/10">
                      <tr>
                        ...
                        <th className="py-2 px-3 text-right text-teal-300">Target Pre-Orders</th>
                    ...
                    <tbody className="divide-y divide-white/5 font-mono text-neutral-300">
                      {preOrdersSummary.daily_breakdown.map((d) => (
                        <tr key={d.date} className="hover:bg-white/5 transition-colors">
                          ...
                          <td className="py-2 px-3 text-right font-bold text-teal-300">{d.total_pre_orders}</td>
              ...
              <div>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    {/* Filter pills */}
                    <div className="flex items-center gap-1 glass-inner p-0.5 rounded-lg border border-white/10 text-[11px]">
                      <button
                        type="button"
                        onClick={() => setClusterStatusFilter('ALL')}
                        className={`px-2 py-0.5 rounded cursor-pointer transition-colors ${clusterStatusFilter === 'ALL' ? 'bg-gradient-to-r from-teal-400 to-emerald-500 text-slate-950 font-bold' : 'text-neutral-400 hover:text-white'}`}
                      >
                      ...
                      <button
                        ...
                        className={`px-2 py-0.5 rounded cursor-pointer transition-colors ${clusterStatusFilter === 'MATCHED' ? 'bg-emerald-500 text-slate-950 font-bold' : 'text-neutral-400 hover:text-white'}`}
                      >
                      ...
                      <button
                        ...
                        className={`px-2 py-0.5 rounded cursor-pointer transition-colors ${clusterStatusFilter === 'MISSING' ? 'bg-teal-500/20 text-teal-300 font-bold border border-teal-500/40' : 'text-neutral-400 hover:text-white'}`}
                      >
                  ...
                  <input
                    ...
                    className="px-2.5 py-1 glass-input rounded-lg text-xs"
                  />
                </div>

                <div className="overflow-x-auto max-h-80 rounded-xl border border-white/10 overflow-y-auto glass-inner">
                  <table className="w-full text-left text-xs">
                    <thead className="glass-inner text-neutral-400 text-[11px] uppercase sticky top-0 border-b border-white/10">
                      <tr>
                    ...
                    <tbody className="divide-y divide-white/5 font-mono text-neutral-300">
                      {preOrdersSummary.clusters
                        ...
                        .slice(0, 150)
                        .map((c) => (
                          <tr key={c.cluster_id} className="hover:bg-white/5 transition-colors">
                            <td className="py-2 px-3 font-bold text-white">
                              <div>{c.client_code}</div>
                              <div className="text-[10px] text-neutral-400 font-normal">{c.cluster_id}</div>
                            </td>
                            <td className="py-2 px-3 text-neutral-300">{c.trade_date}</td>
                            <td className="py-2 px-3 text-neutral-300">
                              {c.start_time}
                              {c.start_time !== c.end_time ? ` → ${c.end_time}` : ''}
                            </td>
                            <td className="py-2 px-3 font-bold text-teal-300 font-sans">{c.symbol}</td>
                            <td className="py-2 px-3 text-right">
                              <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${c.trade_count > 1 ? 'bg-teal-400/20 text-teal-300 border border-teal-400/30' : 'glass-inner text-neutral-400'}`}>
                            ...
                            <td className="py-2 px-3 text-right text-teal-300">₹{c.average_price.toFixed(2)}</td>
                            <td className="py-2 px-3">
                              {c.matched_call_id ? (
                                <button
                                  type="button"
                                  onClick={() => onSelectCall && onSelectCall(c.matched_call_id!)}
                                  className="inline-flex items-center gap-1.5 text-[11px] font-sans font-bold bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/40 px-2.5 py-1 rounded-lg transition-colors cursor-pointer"
                                ...
                                <span className="inline-flex items-center gap-1 text-[11px] font-sans text-teal-400/80 bg-teal-400/10 border border-teal-400/25 px-2 py-0.5 rounded-lg">
      ...
      <div className="glass-panel rounded-2xl shadow-xl overflow-hidden">
        <div className="p-4 border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-neutral-400">Real trade execution data used as factual baseline for matching &amp; Q3 auditing</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="glass-inner text-neutral-300 font-semibold border-b border-white/10 text-[11px] uppercase tracking-wider">
              <tr>
                <th className="py-2.5 px-3 text-teal-300">Trade ID</th>
              ...
              {filteredTrades.length === 0 ? (
                ...
                filteredTrades.map((t) => (
                  <tr key={t.id} className="hover:bg-white/5 transition-colors">
                    <td className="py-2.5 px-3 font-mono font-bold text-teal-400">#{t.id}</td>
                    <td className="py-2.5 px-3 font-mono text-neutral-400">{t.dealer || '—'}</td>
                    <td className="py-2.5 px-3 font-semibold text-neutral-100">{t.advisor_name || '—'}</td>
                    <td className="py-2.5 px-3 text-neutral-400">{t.team || '—'}</td>
                    <td className="py-2.5 px-3">
                      <span className="font-mono font-bold px-2 py-0.5 rounded glass-inner text-teal-300 border border-white/10">
                        {t.client || '—'}
                      </span>
                    </td>
                    <td className="py-2.5 px-3 font-mono text-neutral-300">{t.client_number || t.phone_number || '—'}</td>
                    <td className="py-2.5 px-3 text-neutral-400">{t.trade_date || '—'}</td>
                    <td className="py-2.5 px-3 font-mono text-neutral-400">{t.trade_time || '—'}</td>
                    ...
                    <td className="py-2.5 px-3 text-right font-mono font-bold">
                      <div className="flex flex-col items-end">
                        <span className="text-neutral-100">{t.quantity !== undefined && t.quantity !== null ? t.quantity.toLocaleString() : '—'}</span>
                        {Boolean(t.is_combined) && (
                          <span className="text-[10px] font-sans font-semibold text-teal-300 bg-teal-400/15 border border-teal-400/30 px-1 py-0.2 rounded mt-0.5" title={t.notes || ''}>
                    ...
                    <td className="py-2.5 px-3 text-right font-mono font-bold">
                      <div className="flex flex-col items-end">
                        <span className={t.price_display === 'CMP' ? 'text-cyan-300 bg-cyan-500/20 px-1.5 py-0.5 rounded border border-cyan-500/30 text-xs' : 'text-teal-300'}>
